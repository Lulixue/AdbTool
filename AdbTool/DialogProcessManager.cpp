// DialogProcessManager.cpp : ÊµÏÖÎÄ¼þ
//
...
// CDialogProcessManager ÏûÏ¢´¦Àí³ÌÐò
...
BOOL CDialogProcessManager::OnInitDialog()
{
	...
	//dwStyle |= LVS_EX_CHECKBOXES;//itemÇ°Éú³Écheckbox¿Ø¼þ
...
void CDialogProcessManager::OnPackageoperationsUninstall()
{
	...
	CString info;
	info.Format(TEXT("È·ÈÏÐ¶ÔØ°ü:%s?"), process.NAME);
	int nRet = MessageBox(info, TEXT("Warning"), MB_ICONQUESTION | MB_YESNOCANCEL);
	if (nRet != IDYES) {
		return;
	}
